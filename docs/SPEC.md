...
一次ソースが存在せず二次ソースしかない場合、その事実自体をFinding（info）とし、確信度を下げる。Claimの抽出では一次ソースと二次ソースの**食い違い**（issueはAを要求、PR説明はBを実装したと主張）を明示的に検出し、見つかればFindingとする。
...
**Probe Driver（プラグイン式ドライバ）**: ターゲットの種類ごとに「起動 / 操作 / 観測 / 終了」を実装するドライバを差し替える。すべてのドライバは共通インターフェース（`launch` / `interact` / `observe` / `teardown`）を実装し、Orchestratorからはターゲット種別を意識せず同一に扱える。種別は自動検出（`package.json`のelectron依存、`tauri.conf.json`等）し、設定で上書き可能。
...
- 決定ルール（上から順に評価）:
  ...
  2. 検証手段が実行できなかった（環境構築失敗等）→ `inconclusive`
...
- 確信度は「検証できたClaimの割合（must-verifyを重み付け） × Check手段の強度（実行 > テスト > 読解）」から決定的に算出する。算出式は `verdict.schema.json` とともにバージョン管理する。
...
```markdown
## Verdict: ⚠️ Conditional (confidence: 78/100)
...
| 再現性 | **統計的目標**: 同一入力・同一設定でのN=5回実行でVerdict区分一致率 ≥ 95%（§11.4で実測）。Claim抽出・Finding発見はLLM由来で非決定的なため100%は保証できない。判定ロジック（Stage 6）自体は決定的であり、`--reuse-claims <run-id>` で過去実行のClaimセットを固定して再実行すれば判定差分の原因を切り分けられる |
...
Verifierは「判定」を売るプロダクトであり、その判定の正しさ自体が検証されていなければ信頼に値しない。「検証者を誰が検証するのか」への答えを最初から仕組みとして組み込む。
