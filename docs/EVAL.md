...
| sb-005 | エラーパスでファイルハンドルリーク | logic | conditional | 読解のみ→反証で再現させmajor化（reproConfirmed昇格）（※2） |
| sb-006 | 環境変数のAPIキーをログ出力 | security | not_mergeable | secret scan + security レンズ |
| sb-007 | キャッシュキー衝突（マルチバイト） | logic | conditional | エッジケース再現（※1、※2） |
...
| sb-010 | issueは機能Aを要求、実装はB | — | verdictAnyOf: [conditional, not_mergeable] | 一次/二次ソースの食い違い → 一次ソース由来must Claimのunverified/failed（※2） |

※1: テスト生成はPhase 2機能のため、Phase 1では**反証エージェントの再現コード実行**（Stage 4）で検出する経路を正とする。Phase 2でテスト生成導入後も同じ期待値で通ること（検出経路が増えるだけ）を確認する。

※2: sb-005、sb-007、sb-010は、現在の決定的なMVPパイプライン
（`minimal-verdict.ts`）だけでは意図した検証経路を実装できない。sb-005/007には
Stage 4の反証・再現能力、sb-010には一次ソースからのClaim抽出とdiffとの意味的照合が
必要であり、いずれも`agents`/`judge`を含む後続実装の対象とする。単なるsoft-risk語の
一致で`conditional`にすることは、これらのケースを実装したものとはみなさない。
fixtureを先に追加する場合は期待値を変更せず`knownGap: true`として未実装を可視化し、
...
| flaky-500 | `?fail=1` または `X-FLAKY: true` 指定時だけ500を返す（未指定時は200） | リトライポリシーの検証。例: `GET /health?fail=1` を同じpathでretryして成功させる |
